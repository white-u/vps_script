...
# 修复: 移除静默模式(-s)以显示详细报错，增加 uninstall 接口调用
# Usage: bash vps.sh
...
# TS=$(date +%s)
BASE_URL="https://raw.githubusercontent.com/white-u/vps_script/refs/heads/main"
...
if command -v ptm >/dev/null 2>&1; then
    ...
    # [修复] 使用 -L (重定向) -f (失败报错) --progress-bar (进度条)
    # 不再使用 -s，这样如果出错您能直接看到 curl 的报错信息
