...
**版本**: v5.5.0 | **快捷命令**: `pm` | **安装路径**: `/usr/local/bin/pm`
...
1. 运行 `pm`（也可从 `vt` -> `4. 端口流量监控` 进入），选择 `2. 配置 端口`。
...
卸载会自动清理本脚本的 Cron 任务、Nftables 规则，以及由本脚本创建并登记的 TC 根队列和所有配置文件；其他程序创建的 TC 根队列会保留。
