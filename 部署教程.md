...
部署后，你可以通过 Telegram Bot 集中查询多台 VPS 的流量使用情况，并为合租用户分配查询权限。用户到期前 3 天提醒由各 VPS 上的 PM Telegram 告警负责。
...
1. 保证 VPS 上的 `pm.sh` 已更新到 v5.6.2，Worker 已更新到 v3.0，并在 PM 的 `4. 通知与推送` 中开启 Telegram 告警。
