...
**脚本版本**：v1.2.2　|　**快捷命令**：`sb`　|　**安装路径**：`/usr/local/bin/sb`
...
进入菜单 `6) 跨机出口（A → B）`。菜单中的 A 机是用户实际连接的节点服务器，B 机是负责访问目标网站的出口服务器。

在 B 机：

1. 在 `[B 机：提供出口]` 下选择 `创建中继`。
...
1. 在 `[A 机：使用 B 机出口]` 下选择 `连接、更换或断开 B 机`。
2. 填写 B 机输出的地址和密钥。
3. 选择 `选择要走 B 机的节点`，为指定节点启用该出口。
